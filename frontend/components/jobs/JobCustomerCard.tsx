...
import { HiChatBubbleLeftRight, HiPhone, HiUser } from "react-icons/hi2";
...
export default function JobCustomerCard({ job }: Props) {
  const customerNameDisplay =
    [job.customerFirstName, job.customerLastName].filter(Boolean).join(" ").trim() ||
    job.customerName ||
    "—";
  const customerPhoneDigits = job.customerPhone ? phoneDigitsForLinks(job.customerPhone) : "";
